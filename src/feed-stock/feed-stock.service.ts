...
export class FeedStockService {
  ...
  async gettingAllFeedRecords(
    ...
    try {
      ...
      let allRecords = await this.prisma.feedStock.findMany({
        orderBy: { date: 'desc' },
        skip: skip,
        take: limit,
      });
      ...
      if (sortBy) {
        message = 'showing the sorted feed stock data';
        switch (sortBy) {
          case 'name-asc':
      ...
      // Unit filter handling
      if (filter && Array.isArray(filter) && filter.length > 0) {
        const units: SelectedUnit[] = [];

        filter.forEach((f) => {
          const upper = f.toUpperCase();
          if (Object.values(SelectedUnit).includes(upper as SelectedUnit)) {
        ...
        if (units.length > 0) {
          ...
          message = `Showing filtered data based on selected unit(s)`;
        }
      }
      ...
      if (fromDate && toDate) {
        ...
        message = filter?.length
          ? `Showing filtered data for selected unit(s) from ${fromDate} to ${toDate}`
          : `Showing filtered data from ${fromDate} to ${toDate}`;
      }

      // Only query if there's any filter applied (unit, date, or both)
      ...
      if (search) {
        ...
        totalCount = await this.prisma.feedStock.count({
          where: {
            name: {
              contains: search,
              mode: 'insensitive',
  ...
  async gettingParticularStockRecordHistory(id: number) {
    try {
      const allStockRecords = await this.prisma.feedStockHistory.findMany({
        where: { feedId: id },
        select: {
          id: true,
          feedId: true,
      ...
      return {
        message: `Showing all history of feed ${allStockRecords?.[0].feedStock.name}`,
        allStockRecords,
      };
    } catch (error) {
      catchBlock(error);
  ...
  async gettingAllFeedStockNames() {
    try {
      ...
      return {
        message: 'Showing all the stock names quantity greater than 0',
        allStockNames,
      };
    } catch (err) {
      catchBlock(err);
